...
# mat_upr <- mat
# q05 <- quantile(mat_upr, 0.05)
# mat_upr[mat_upr > q05] <- NA
...
### > 0.40
df50 <- data.frame(chunk = df$chunk[df$value <= q05])
df50$lag <- df$lag[df$value <= q05]
...
# patterns <- list(df50[c(260, 261, 246, 247, 248, 249), ],
#                  df50[c(265, 251, 252), ],
#                  df50[c(268, 254, 255), ],
#                  df50[c(270, 271, 258), ])#,
#                  df50[c(15, 8), ])
...
patterns <- df50[-c(1), ]
patterns <- patterns[order(patterns$chunk), ]
###########################################################################
...
rsp <- lapply(seq(nrow(patterns)), function(i) {
    ptrn <- paste("*", paste("kili", patterns[i, ]$chunk,
                             patterns[i, ]$lag, sep = "_"),
                  ".rda", sep = "")
    print(ptrn)
    fl <- list.files("five_yr_chunks/results/modes",
                     pattern = ptrn, full.names = TRUE)
...
plts_rsp <- lapply(seq(rsp), function(i) {
    spplot(rsp[[i]], col.regions = clrs_rsp(1000),
           at = seq(-0.8, 0.8, 0.01))
})
...
### load modes
mds <- lapply(seq(nrow(patterns)), function(i) {
...
# png("five_yr_chunks/graphs/patterns_lwr5.png",
#      width = 174, height = 174, units = "mm", res = 600)
# latticeCombineGrid(plts, layout = c(3, 5)) +
#   layer(grid.text(x = unit(0.01, "npc"),
#                   y = unit(0.05, "npc"),
#                   just = c("left", "bottom"),
#                   label = paste(patterns[panel.number(), ]$chunk, "\n",
#                                 patterns[panel.number(), ]$lag),
#                   gp = gpar(col = "white", cex = 0.7)))
# dev.off()

tiff("five_yr_chunks/graphs/patterns_lwr5.tif",
    width = 174, height = 174, units = "mm", res = 600,
    compression = "lzw")
latticeCombineGrid(plts, layout = c(3, 5)) +
  layer(grid.text(x = unit(0.01, "npc"),
