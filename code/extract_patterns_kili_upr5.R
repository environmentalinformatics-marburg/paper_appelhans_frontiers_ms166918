...
x_at <- c(3, 1, 1, 3, 4, 6, 7, 12, 10, 11, 11, 11)
y_at <- c(13, 12, 11, 11, 11, 11, 11, 13, 12, 12, 11, 13)
labs <- c("a)", "b)", "b)", "c)", "c)", "d)", "d)", "e)", "e)", "e)", "e)", "e)*")
...
#   print(i)
#   spplot(mds[[i]], col.regions = clrs(1000),
#          at = seq(0, 1, 0.01))
# })
#
...
### > 0.40
df50 <- data.frame(chunk = df$chunk[df$value >= q95])
df50$lag <- df$lag[df$value >= q95]
...
plts <- lapply(seq(mds), function(i) {
  lst <- lapply(seq(nlayers(mds[[i]])), function(j) {
    spplot(mds[[i]][[j]], col.regions = clrs(1000),
           at = seq(0, 1, 0.01))
  })
...
tsh <- c(0.75, 0.75, 0.75, 0.75, 0.75)
...
plts_mn <- lapply(seq(mds), function(i) {
  ...
  labs <- function(x) letters[x]

  mn <- mean(mds[[i]])
  mx <- which.max(mn)
  xy.max <- xyFromCell(mn, mx)

  spplot(mean(mds[[i]]), col.regions = clrs(1000),
         at = seq(0, 1, 0.01),
         colorkey = FALSE, panel = function(...) {
           panel.levelplot(...)
           panel.text(x = -170, y = 50, col = "white", font = 2,
                      labels = paste0(letters[i], ")"))
           }) + #list(space = "left", height = 1/6*2, x = 0)) +
    layer(panel.map("world", border = "white", col = "grey60", lwd = 0.5)) +
    #layer(panel.extent(extent(ext), border = "black", lwd = 2, lty = 1)) +
    as.layer(spplot(mean(mds[[i]]), col.regions = "transparent",
                     at = seq(tsh[i], 1, length.out = 2), contour = TRUE)) +
    as.layer(xyplot(xy.max[, 2] ~ xy.max[, 1], pch = 23,
                    col = "black", fill = "white", cex = 1))
})
...
all_sst <- lapply(seq(patterns), function(i) {
  ptrn <- paste("*", sq, "*", ".rst", sep = "")
  out <- stck_sst_dsn_dns[[which(substr(names(stck_sst_dsn_dns),
                                        5, 8) %in% sq)]]
  print(names(out))
...
sq <- seq(min(as.numeric(ts_vals$st)), max(as.numeric(ts_vals$nd)), 1)
all_gpcp <- lapply(seq(patterns), function(i) {
  ptrn <- paste("*", sq, "*", ".rst", sep = "")
  out <- stck_prcp_kili_dsn_dns[[which(substr(names(stck_prcp_kili_dsn_dns),
                                              13, 16) %in% sq)]]
  print(names(out))
...
ts_lst <- lapply(seq(patterns), function(i) {
  ...
  if (length(cells) == 0) return(NULL) else {
    ...
    sst_ts <- apply(X = extract(sst_st, cells), MARGIN = 2,
                    FUN = mean, na.rm = TRUE)
    gpcp_ts <- apply(all_gpcp[[i]][], 2, mean, na.rm = TRUE)
...
ts_reg <- lapply(seq(ts_lst[1:5]), function(i) {
  print(i)
  return(list(fit = auto.arima(rsp, xreg=ts_lst[[i]]$sst),#
              lm1 = lm(rsp ~ ts_lst[[i]]$sst)))
})

rsq <- list(summary(ts_reg[[1]]$lm1)$r.squared,
            summary(ts_reg[[2]]$lm1)$r.squared,
            summary(ts_reg[[3]]$lm1)$r.squared,
            summary(ts_reg[[4]]$lm1)$r.squared,
            summary(ts_reg[[5]]$lm1)$r.squared)
#######################


bls <- brewer.pal(5, "Blues")[c(3, 5)]
...
ts_plt_lst <- lapply(seq(patterns), function(i) {
  ...
  if (length(cells) == 0) return(NULL) else {
    ...
    sst_ts <- ts(apply(X = extract(chnks_sst[[i]], cells), MARGIN = 2,
                    FUN = mean, na.rm = TRUE))
    sst_ts_all <- ts(apply(X = extract(all_sst[[i]], cells), MARGIN = 2,
                        FUN = mean, na.rm = TRUE))
    gpcp_ts <- apply(chnks_gpcp[[i]][], 2, mean, na.rm = TRUE)
    gpcp_ts_all <- apply(all_gpcp[[i]][], 2, mean, na.rm = TRUE)
    x_by <- ceiling(mean(as.numeric(substr(patterns[[i]]$lag, 4, 5))))
    print(x_by)
    #   if (i == 6) {
    ...
    yrs_all <- substr(names(sst_ts_all), 5, 8)
    mnths_all <- substr(names(sst_ts_all), 10, 11)
    yrs_chnk <- substr(names(sst_ts), 5, 8)
    mnths_chnk <- substr(names(sst_ts), 10, 11)
    dat_all <- data.frame(date = as.Date(paste(yrs_all, mnths_all, "01",
                                               sep = "-")),
                          sst = as.numeric(sst_ts_all),
                          prcp = as.numeric(gpcp_ts_all) / 100)
    dat_chnk <- data.frame(date = as.Date(paste(yrs_chnk, mnths_chnk, "01",
                                                sep = "-")),
                           sst = as.numeric(sst_ts),
                           prcp = as.numeric(gpcp_ts) / 100)

    sst_ts_srt <- sort(abs(dat_chnk$sst), decreasing = TRUE)
    st1 <- dat_chnk$date[as.numeric(which(abs(dat_chnk$sst) == sst_ts_srt[1]))]
    st2 <- dat_chnk$date[as.numeric(which(abs(dat_chnk$sst) == sst_ts_srt[2]))]
    nd1 <- st1
    month(nd1) <- month(st1) + x_by
    nd2 <- st2
    month(nd2) <- month(nd2) + x_by
    labs_st <- substr(names(sst_ts)[1], 5, 8)
    labs_nd <- substr(names(sst_ts)[length(sst_ts)], 5, 8)

    xyplot(sst ~ date, data = dat_all, type = "l", lwd = 2,
           col = "grey70", ylim = c(-2.3, 2.3),
           panel = function(...) {
             if (x_by > 0) {
...
#                              col = "grey90", border = "grey30")
               ...
               panel.abline(v = c(st1), lwd = 2, col = "grey")
             }
             #            panel.abline(v = seq(st_ovr, nd_ovr, 1), lty = 1,
             #                         lwd = 0.5, col = "grey")
             panel.xyplot(...)
             panel.text(x = rep(as.Date("1992-10-01"), 7),
                        y = c(-3:3),
                        labels = as.character(-3:3), cex = 0.7)
             panel.text(x = as.Date("1993-06-01"),
                        y = 1.8, adj = c(0, 0.5), cex = 0.7,
                        labels = paste(labs_st, labs_nd, sep = " - "))
             panel.text(x = as.Date("1999-06-01"),
                        y = 1.8, cex = 0.7,
                        labels = bquote(italic(R)^2 ==
                                          .(format(rsq[[i]],
                                                   digits = 2))))
           }) +
      as.layer(xyplot(prcp ~ date, data = dat_all, col = bls[1],
                      type = "l", lty = 3, lwd = 2)) +
      as.layer(xyplot(sst ~ date, data = dat_chnk, type = "l",
                      col = "black", lwd = 2)) +
      as.layer(xyplot(prcp ~ date, data = dat_chnk, type = "l",
                      col = bls[2], lwd = 2, lty = 3)) +
      layer_(panel.abline(h = 0, lty = 2, col = "grey")) +
      layer_(panel.abline(h = c(1, 2, 3), lty = 1, col = "grey")) +
      layer_(panel.abline(h = c(-1, -2, -3), lty = 1, col = "grey")) +
      layer_(panel.abline(v = seq.Date(as.Date("1993-01-01"),
                                       as.Date("1999-12-01"),
                                       by = "month"),
                                       lwd = 0.3, col = "grey80"))
  }
...
final <- latticeCombineGrid(list(plts_mn[[1]], ts_plt_lst[[1]],
                                 plts_mn[[2]], ts_plt_lst[[2]],
                                 plts_mn[[3]], ts_plt_lst[[3]],
...
#      compression = "lzw")
png("five_yr_chunks/graphs/patterns_mode1_w_ts.png",
     width = 174, height = 234, units = "mm", res = 600)
grid.newpage()
print(final)
...
grid.lines(x = 0.75, y = c(0,1), draw = TRUE)
upViewport(n = 2)
...
summary(ts_reg[[1]]$lm1)$r.squared
summary(ts_reg[[2]]$lm1)$r.squared
summary(ts_reg[[3]]$lm1)$r.squared
summary(ts_reg[[4]]$lm1)$r.squared
summary(ts_reg[[5]]$lm1)$r.squared
